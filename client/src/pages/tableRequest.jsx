...
import { Books, Requests } from "../utils/table_fields";
...
const TableRequest = () => {
    ...
    const [fetchData, isDataLoading, dataError] = useFetching(async (str) => {
        switch (str) {
            case 'list':
                setProccess(true)
                const list = await RequestList.getAll('requests', limit, page);
                setData([...list.data[0]])
            ...
            case 'filter':
                setProccess(true)
                const searchList = await RequestList.filterItems('requests', filter.query, filter.sort, limit, page);
    ...
    return (
        <article style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-start',alignItems: 'center', background: '#191615', padding: '0 20px 40px 20px', }} className="post">
            ...
            <SelectPicker
                ...
                data={[{ value: 'numberIncrease', label: 'По тиражу ↑' },
